<!-- ::: Footer ::: -->
<footer class="footer">
  <div class="container footer__wrapper textAlign-center textAlign-md-left">
    <div class="footer__address">
      <h3 class="marginBottom-4 fontSize-3">Contáctanos</h3>

      <address class="fontSize-2">
        <div class="marginBottom-1">
          <p><i class="fa-fw fa-solid fa-location-dot marginRight-1"></i> Dr. Mariano Azuela 111, Col. Santa María la
            Ribera, Del. Cuauhtemoc, México.</p>
        </div>
        <div class="marginBottom-1">
          <p><i class="fa-fw fa-solid fa-phone marginRight-1"></i> 55-55-47-72-19 y 55-55-47-02-74</p>
        </div>
        <div>
          <a href="mailto:elcorreoquequieres@correo.com"><i class="fa-fw fa-solid fa-envelope marginRight-1"></i>
            info@qualitybakery.com</a>
        </div>
      </address>
    </div>

    <div class="footer__logo">
      <img loading="lazy" src="./assets/img/quality-bakery-logo.png" alt="logo de quality bakery" class="marginBottom-1" />

      <nav aria-label="redes sociales">
        <ul class="inline-list">
          <li>
            <a class="marginRight-1" href="https://www.facebook.com/Qualitybakeryoficial/"
              aria-label="ir a perfil de facebook de quality bakery">
              <i class="fa-fw fa-brands fa-facebook"></i>
            </a>
          </li>
          <li>
            <a class="marginRight-1" href="https://www.instagram.com/qualitybakery_oficial/?hl=es"
              aria-label="ir a perfil de instagram de quality bakery">
              <i class="fa-fw fa-brands fa-instagram"></i>
            </a>
          </li>
          <li>
            <a href="https://www.tiktok.com/es/" aria-label="ir a perfil de tiktok de quality bakery">
              <i class="fa-fw fa-brands fa-tiktok"></i>
            </a>
          </li>
        </ul>
      </nav>
    </div>

    <div class="footer__about">
      <h3 class="marginBottom-4 fontSize-3">Quality Bakery</h3>

      <p class="fontSize-2">
        Somos una empresa mexicana dedicada a la pastelería tradicional, fusionamos nuestro sabor con el toque
        tradicional que caracteriza a la pastelería mexicana.
      </p>
    </div>
  </div>
</footer>

</body>

</html>